...
## Notes for downstream workstreams (WS-B through WS-J)

- **Provider nesting** (`src/app/providers.tsx`): `ThemeProvider` (next-themes,
  `attribute="class"`) → `QueryClientProvider` → `TooltipProvider` → children,
  with the toast `<Toaster />` rendered as a sibling of children. `QueryClient`
  is created in `useState` (SSR-safe, not a module singleton). Defaults:
  `refetchOnWindowFocus: false`, `retry: 1`, `staleTime: 30_000` — tuned for a
  backend that costs 2.7–7.4s warm (and sometimes real LLM spend) per call;
  override per-query where a shorter interval is needed (e.g. `/ingest/{id}`
  polling, `/health`).
- **`<main>` landmark:** lives in `src/components/layout/app-shell.tsx`,
  rendered once by `src/app/(app)/layout.tsx`. The root `layout.tsx` has no
  `<main>` and no nav on purpose, so `(auth)/*` pages (WS-C) can supply their
  own minimal shell without inheriting app chrome — give that shell its own
  single `<main>` too, don't nest inside `AppShell`'s.
- **Theme control:** `next-themes`, toggled by `src/components/layout/theme-toggle.tsx`,
  flips the `.dark` class on `<html>`. All design tokens (including verdict
  tokens) key off that class in `globals.css` — don't read `prefers-color-scheme`
  directly anywhere else.
- **Auth affordance in the header:** `src/components/layout/header.tsx` has no
  user menu / login link yet (that's WS-C, `src/components/auth/`). WS-A →
  WS-B → WS-C is a serial dependency chain, so WS-C editing `header.tsx` to
  slot in a user menu is expected, not a collision.
- **Shared states:** `ErrorState` and `EmptyState`
  (`src/components/layout/error-state.tsx`, `empty-state.tsx`) plus
  `TextSkeleton` / `CardSkeleton` / `ListSkeleton` (`skeletons.tsx`) are the
  idle/loading/error/empty building blocks every API-touching component must
  use (§5.3) — don't hand-roll new ones. `ErrorBoundary`
  (`error-boundary.tsx`) is for render-time crashes, not query failures;
  query failures render `ErrorState` directly from TanStack Query's `error`.
- **shadcn primitives are built on Base UI**, not Radix directly (current
  `shadcn@latest` default) — same accessibility guarantees (keyboard nav,
  ARIA) the plan assumed from Radix, different import path if you ever need
  a primitive shadcn doesn't wrap.
- **Alias:** `@/*` → `src/*`, configured in both `tsconfig.json` and
  `vitest.config.ts` — keep them in sync if either changes.
- **`noUncheckedIndexedAccess` is on.** Any array index access
  (`citations[i]`, `chunk_ids[i]`, etc.) returns `T | undefined` — this is
  deliberate, not a bug to silence with `!`.
